...
public class Sorter {
    ...
    public <T> void sort(List<T> list, String[] properties, SortOrder sortOrder)
        ...
        for(int idxProp = 1 ; idxProp < properties.length  ; idxProp++ ) {
            ...
            for( int idxObj = 1 ; idxObj < list.size(); idxObj ++) {
                if( comparator.compare(list.get(idxObj - 1), 
                        list.get(idxObj)) == 0 ) {
     ...
     * @param propertyClass the Class that need to be used to compare in an 
     * object property
     ...
     * @deprecated use registerBeanComparator(Class propertyClass, 
     * Class beanComparatorClass) to replace
     */
    public void registerBeanComparator(Class propertyClass, 
